...
class GraphContext:
    ...
    graph_index: GraphIndex | None = field(default=None)
    ...
    def create(cls, settings: Settings) -> GraphContext:
        ...
        graph_index = GraphIndex(graph_root / "nodes")
        graph_index.build()
        return cls(
    ...
    def for_worktree(self, worktree_path: Path) -> GraphContext:
        """New context pointing at a worktree. Chromadb + OpenAI client shared; no graph_index (worktrees are ephemeral)."""
        return GraphContext(
